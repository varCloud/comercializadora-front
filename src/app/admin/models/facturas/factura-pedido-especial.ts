// Renglón del listado "Facturas Pedidos Especiales", tal como lo devuelve la API
// (GET /api/facturas/pedidos-especiales). Hermana de FacturaVenta (misma forma, cambia el
// identificador: idPedidoEspecial en vez de idVenta). Un archivo = una interfaz + su modelo
// (regla 11). Nombres de campo = contrato JSON de la API.

export interface FacturaPedidoEspecial {
  idFacturaPedidoEspecial: number;
...
export class FacturaPedidoEspecialModel implements FacturaPedidoEspecial {
  idFacturaPedidoEspecial: number;
  idPedidoEspecial: number;
  fecha: string | null;
  fechaTimbrado: string | null;
  ...
  constructor(data: Partial<FacturaPedidoEspecial> = {}) {
    this.idFacturaPedidoEspecial = data.idFacturaPedidoEspecial ?? 0;
