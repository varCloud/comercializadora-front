...
import { CargaMercanciaLiquidos } from 'src/app/admin/models/produccion-liquidos/carga-mercancia-liquidos';
import { ProduccionLiquidosService } from 'src/app/admin/services/produccion-liquidos.service';
import { UsuariosService } from 'src/app/admin/services/usuarios.service';

/**
 * Roles habilitados para este reporte (paridad con `ConsultarLiquidos`/`ConsultarUsuariosLiquidos`
 * del legado, que solo permitía filtrar por los roles 12/13). No hay endpoint propio: se filtra
 * el catálogo general de roles de `UsuariosService` (regla 00 — reuso, no se crea catálogo nuevo).
 */
const ROLES_LIQUIDOS = [12, 13];

/**
 * Reporte de solo lectura "Producción Líquidos" (carga de mercancía). Migra
 * ConsultarLiquidos/BuscarCargaMercanciaLiquidos del legado: filtra por rol, usuario (en cascada
 * por rol) y rango de fechas; sin alta/edición/baja y sin buscador de texto libre (regla 13 no
 * aplica: filtros estructurados). Reusa UsuariosService para los catálogos de rol/usuario.
 */
@Component({
  selector: 'app-produccion-liquidos-list',
  standalone: true,
  imports: [
    MaterialModule,
    MatNativeDateModule,
  ...
  templateUrl: './produccion-liquidos-list.component.html',
  styleUrl: './produccion-liquidos-list.component.scss',
})
export class ProduccionLiquidosListComponent implements OnInit {
  private readonly service = inject(ProduccionLiquidosService);
  private readonly usuariosService = inject(UsuariosService);
  private readonly notify = inject(NotificationService);
  private readonly translate = inject(TranslateService);
  private readonly fb = inject(FormBuilder);

  @BlockUI('produccionLiquidos') blockUI!: NgBlockUI;
  ...
  readonly pag = new Paginador<CargaMercanciaLiquidos>(CONSTANTS.PAGINATION.PAGE_SIZE);
  ...
  ngOnInit(): void {
    this.usuariosService.obtenerRoles().subscribe({
      next: (roles) => this.roles.set(roles.filter((r) => ROLES_LIQUIDOS.includes(r.id))),
  ...
  private cargarUsuarios(idRol: number | null): void {
    const roles = idRol ? [idRol] : ROLES_LIQUIDOS;
  ...
  cargar(): void {
    this.blockUI.start(this.translate.instant('produccionLiquidos.msg.loading'));
    this.service
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('Error al listar producción de líquidos', err);
          this.notify.notify('error', this.translate.instant('produccionLiquidos.msg.loadError'));
        },
      });
  }
  ...
  navegar(url: string): void {
    ...
    this.service
      ...
      .subscribe({
        next: (res) => this.pag.setPage(res),
        error: (err) => {
          console.error('Error al paginar producción de líquidos', err);
          this.notify.notify('error', this.translate.instant('produccionLiquidos.msg.loadError'));
        },
      });
  }
