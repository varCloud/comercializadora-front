import { Component, OnInit, inject, signal } from '@angular/core';
import { ReactiveFormsModule, FormBuilder, Validators } from '@angular/forms';
import { MAT_DIALOG_DATA, MatDialogRef } from '@angular/material/dialog';
import { TablerIconsModule } from 'angular-tabler-icons';
import { TranslatePipe, TranslateService } from '@ngx-translate/core';
import { finalize } from 'rxjs';
import { MaterialModule } from 'src/app/material.module';
import { CONSTANTS } from 'src/app/config/constants';
import { NotificationService } from 'src/app/services/notification.service';
import {
  ENUM_ESTATUS_MODAL,
  ResultModalModel,
} from 'src/app/models/result-modal';
import { Catalogo } from 'src/app/admin/models/shared/catalogo';
import { Usuario } from 'src/app/admin/models/usuarios/usuario';
import { GuardarUsuarioRequestModel } from 'src/app/admin/models/usuarios/guardar-usuario-request';
import { UsuariosService } from 'src/app/admin/services/usuarios.service';

/** Datos que recibe el diálogo al abrirse. */
export interface UsuarioFormData {
  usuario?: Usuario;
  /** Solo lectura (modo "ver"). */
  readonly?: boolean;
}

@Component({
  selector: 'app-usuario-form-dialog',
  standalone: true,
  imports: [ReactiveFormsModule, MaterialModule, TablerIconsModule, TranslatePipe],
  templateUrl: './usuario-form-dialog.component.html',
})
export class UsuarioFormDialogComponent implements OnInit {
  private readonly fb = inject(FormBuilder);
  private readonly service = inject(UsuariosService);
  private readonly notify = inject(NotificationService);
  private readonly translate = inject(TranslateService);
  private readonly dialogRef = inject(MatDialogRef<UsuarioFormDialogComponent>);
  readonly data = inject<UsuarioFormData>(MAT_DIALOG_DATA);

  readonly roles = signal<Catalogo[]>([]);
  readonly sucursales = signal<Catalogo[]>([]);
  readonly almacenes = signal<Catalogo[]>([]);
  readonly saving = signal(false);
  readonly hidePassword = signal(true);

  readonly isEdit = !!this.data?.usuario;
  readonly readonly = !!this.data?.readonly;

  readonly form = this.fb.group({
    usuario: ['', Validators.required],
    contrasena: [''],
    nombre: ['', Validators.required],
    apellidoPaterno: [''],
    apellidoMaterno: [''],
    telefono: [''],
    // Sucursal fija en Uruapan y bloqueada (regla 15).
    idSucursal: [{ value: CONSTANTS.SUCURSAL_DEFAULT.ID, disabled: true }, Validators.min(1)],
    idAlmacen: [{ value: 0, disabled: true }, Validators.min(1)],
    idRol: [0, Validators.min(1)],
  });

  get titleKey(): string {
    if (this.readonly) return 'usuarios.form.titleView';
    return this.isEdit ? 'usuarios.form.titleEdit' : 'usuarios.form.titleAdd';
  }

  ngOnInit(): void {
    if (!this.isEdit) {
      this.form.controls.contrasena.addValidators(Validators.required);
    }

    this.service.obtenerRoles().subscribe((r) => this.roles.set(r));
    this.service.obtenerSucursales().subscribe((s) => this.sucursales.set(s));

    const u = this.data?.usuario;
    if (u) {
      this.form.patchValue({
        usuario: u.usuario,
        nombre: u.nombre,
        apellidoPaterno: u.apellidoPaterno ?? '',
        apellidoMaterno: u.apellidoMaterno ?? '',
        telefono: u.telefono ?? '',
        idRol: u.idRol,
      });
      // Sucursal fija (regla 15): carga los almacenes de Uruapan y selecciona el del usuario.
      this.cargarAlmacenes(CONSTANTS.SUCURSAL_DEFAULT.ID, u.idAlmacen);
    } else {
      this.cargarAlmacenes(CONSTANTS.SUCURSAL_DEFAULT.ID);
    }

    if (this.readonly) {
      this.form.disable();
    }
  }

  onSucursalChange(idSucursal: number): void {
    this.form.controls.idAlmacen.setValue(0);
    this.cargarAlmacenes(idSucursal);
  }

  guardar(): void {
    if (this.form.invalid) {
      this.form.markAllAsTouched();
      return;
    }

    const raw = this.form.getRawValue();
    const request = new GuardarUsuarioRequestModel({
      idUsuario: this.data?.usuario?.idUsuario ?? 0,
      usuario: raw.usuario ?? '',
      contrasena: raw.contrasena || null,
      telefono: raw.telefono || null,
      nombre: raw.nombre ?? '',
      apellidoPaterno: raw.apellidoPaterno || null,
      apellidoMaterno: raw.apellidoMaterno || null,
      idRol: raw.idRol ?? 0,
      idSucursal: raw.idSucursal ?? 0,
      idAlmacen: raw.idAlmacen ?? 0,
      activo: this.data?.usuario?.activo ?? true,
    });

    const peticion$ = this.isEdit
      ? this.service.actualizar(request.idUsuario, request)
      : this.service.crear(request);

    this.saving.set(true);
    peticion$.pipe(finalize(() => this.saving.set(false))).subscribe({
      next: (res) => {
        if (res?.estatus === 200) {
          this.notify.notify('success', res.mensaje ?? '');
          this.dialogRef.close(
            new ResultModalModel({
              status: ENUM_ESTATUS_MODAL.OK,
              message: res.mensaje,
            }),
          );
        } else {
          this.notify.notify(
            'error',
            res?.mensaje ?? this.translate.instant('usuarios.msg.saveFallback'),
          );
        }
      },
      error: (err) => {
        console.error('Error al guardar usuario', err);
        this.notify.notify('error', this.translate.instant('usuarios.msg.saveError'));
      },
    });
  }

  cancelar(): void {
    this.dialogRef.close(
      new ResultModalModel({ status: ENUM_ESTATUS_MODAL.CANCEL }),
    );
  }

  private cargarAlmacenes(idSucursal: number, seleccionado?: number): void {
    const control = this.form.controls.idAlmacen;
    if (!idSucursal || idSucursal <= 0) {
      this.almacenes.set([]);
      control.disable();
      return;
    }
    this.service.obtenerAlmacenes(idSucursal).subscribe((a) => {
      this.almacenes.set(a);
      if (!this.readonly) {
        control.enable();
      }
      if (seleccionado) {
        control.setValue(seleccionado);
      }
    });
  }
}
