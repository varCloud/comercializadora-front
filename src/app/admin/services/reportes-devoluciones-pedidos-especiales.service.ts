...
import {
  DevolucionPeReporteItem,
  DevolucionPeReporteItemModel,
} from 'src/app/admin/models/reportes-devoluciones-pedidos-especiales/devolucion-pe-reporte-item';

/** Filtros del listado/exportación de "Reportes > Devoluciones Pedidos Especiales" (la
 * exportación sigue sin paginar). Sin `idLineaProducto` ni buscador de texto libre:
 * `SP_CONSULTA_DEVOLUCIONES_PEDIDOS_ESPECIALESV2` no los soporta (regla 13, excepción
 * sub-reportes de Reportes). */
export interface DevolucionPeReporteFiltros {
...
const NOMBRE_REPORTE = 'ReporteDevolucionesPedidosEspeciales';

/**
 * Servicio HTTP de "Reportes > Devoluciones Pedidos Especiales". Migra
 * `ReportesController.DevolucionesPedidosEspeciales/BuscarDevolucionesPedidosEspeciales` del
 * legado (`SP_CONSULTA_DEVOLUCIONES_PEDIDOS_ESPECIALESV2` tal cual, sin `SP_V2_*` nuevo).
 *
 * Mismo patrón que `ReportesVentasPedidosEspecialesService` (paginación en memoria desde el
 * inicio: el SP no soporta `OFFSET/FETCH`, `Notificacion<T[]>` con `links`/`meta` vía
 * `IPaginationBuilder`). `exportar()` reusa el mismo patrón dual `Descarga`/`Diferido` (sniff
 * de `Content-Type` sobre un blob) — ver `ReportesVentasPedidosEspecialesService.exportar` para
 * el detalle documentado.
 *
 * Reemplaza el mock de `DevolucionesPeListComponent` (FE-2) — ver FE-4 en
 * `task_reporte_devoluciones_pedidos_esp.md`.
 */
@Injectable({ providedIn: 'root' })
export class ReportesDevolucionesPedidosEspecialesService {
  private readonly http = inject(HttpClient);
  private readonly notify = inject(NotificationService);
  private readonly baseUri =
    `${environment.BASE_URL_ADMIN}/${URIS_CONFIG.REPORTES}/${URIS_CONFIG.REPORTE_DEVOLUCIONES_PEDIDOS_ESP}`;

  /** Listado paginado. Filtros: idPedidoEspecial, idAlmacen, idUsuario, fechaIni, fechaFin. */
  listar(opts: ListarParams = {}): Observable<PagedResult<DevolucionPeReporteItem>> {
    const params = buildListParams(opts);
    return this.http
      .get<Notificacion<DevolucionPeReporteItem[]>>(this.baseUri, { params })
      .pipe(map((res) => this.mapPage(res)));
  }

  /** Navega a una URL de paginación (link first/prev/next/last que devolvió la API). */
  irLink(url: string): Observable<PagedResult<DevolucionPeReporteItem>> {
    return this.http
      .get<Notificacion<DevolucionPeReporteItem[]>>(url)
      .pipe(map((res) => this.mapPage(res)));
  }
   ...
   */
  exportar(filtros: DevolucionPeReporteFiltros = {}): Observable<void> {
  ...
  private buildParams(filtros: DevolucionPeReporteFiltros): HttpParams {
    let params = new HttpParams();
    if (filtros.idPedidoEspecial) params = params.set('idPedidoEspecial', filtros.idPedidoEspecial);
  ...
  private mapPage(res: Notificacion<DevolucionPeReporteItem[]>): PagedResult<DevolucionPeReporteItem> {
    return {
      data: (res?.modelo ?? []).map((i) => new DevolucionPeReporteItemModel(i)),
