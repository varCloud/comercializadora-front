...
} from 'src/app/admin/models/reportes-ventas/venta-reporte-item';

/** Filtros del listado/exportación de "Reportes > Ventas" (la exportación sigue sin paginar). */
...
const NOMBRE_REPORTE = 'ReporteVentas';

/**
 * Servicio HTTP de "Reportes > Ventas". Migra `ReportesController.Ventas/BuscarVentas` del
 * legado (`SP_CONSULTA_VENTAS` tal cual, sin `SP_V2_*`).
 *
 * **Corrección post-aprobación (2026-07-19):** `listar()` pasa de devolver el arreglo plano a
 * `PagedResult<VentaReporteItem>` — excepción puntual a "módulo Reportes sin modernizar": el SP
 * legado no se toca, pero un rango de fechas amplio podía traer miles de filas de golpe, así que
 * la API ahora pagina en memoria (`IPaginationBuilder`, `Notificacion<T[]>` con `links`/`meta`,
 * mismo contrato que `ReportesInventarioService.listar`). `exportar()` **no cambia**: sigue sin
 * paginar y sigue respetando los filtros activos de pantalla (a diferencia de Inventario, no
 * tiene el modo "exportar todo").
 *
 * `exportar()` reusa el mismo patrón dual `Descarga`/`Diferido` (sniff de `Content-Type` sobre un
 * blob) que `ReportesInventarioService.exportar` — ver ese archivo para el detalle documentado.
 */
...
export class ReportesVentasService {
  ...
  private readonly baseUri =
    `${environment.BASE_URL_ADMIN}/${URIS_CONFIG.REPORTES}/${URIS_CONFIG.REPORTE_VENTAS}`;

  /** Listado paginado. Filtros: idLineaProducto, idCliente, idUsuario, fechaIni, fechaFin. */
  listar(opts: ListarParams = {}): Observable<PagedResult<VentaReporteItem>> {
    const params = buildListParams(opts);
    return this.http
      .get<Notificacion<VentaReporteItem[]>>(this.baseUri, { params })
      .pipe(map((res) => this.mapPage(res)));
  }

  /** Navega a una URL de paginación (link first/prev/next/last que devolvió la API). */
  irLink(url: string): Observable<PagedResult<VentaReporteItem>> {
    return this.http
      .get<Notificacion<VentaReporteItem[]>>(url)
      .pipe(map((res) => this.mapPage(res)));
  }

  /**
   * Exporta a CSV con los MISMOS filtros del listado (a diferencia de Inventario, no ignora los
   * filtros de pantalla). Respuesta dual: `200 File` (descarga inmediata) o
   * `200 Notificacion<string>` (envío diferido, llega como Blob por el `responseType: 'blob'`
   * fijo; se convierte a texto y se parsea). Los errores (400: sin correo registrado) también
   * llegan con `error.error` como Blob por el mismo motivo.
   */
  exportar(filtros: VentaReporteFiltros = {}): Observable<void> {
  ...
  private buildParams(filtros: VentaReporteFiltros): HttpParams {
  ...
  private mapPage(res: Notificacion<VentaReporteItem[]>): PagedResult<VentaReporteItem> {
    return {
      data: (res?.modelo ?? []).map((i) => new VentaReporteItemModel(i)),
